...
def add_mesh_to_scene(scene: Scene, pypath: Path, field_components=("disp_x","disp_y", "disp_z"), fields_to_render = ("disp_y", "disp_x"), world_position: np.ndarray = None, scale: float = 100.0) -> None:
    ...
    """ Commented out for data access tests to keep single timestep at all times"""
    if timestep_count != 1:
    ...
    scene.add_mesh(node_coords_expanded_over_time, coords, connectivity, face_colors_over_time, timestep_count)
