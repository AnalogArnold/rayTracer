...
// Stack-based iterative implementation of BVH TLAS and BLAS, derived from the recursive version. Still using pointers
...
// Bounding volume structure - axis-aligned bounding boxes (AABB_it)
struct AABB_it {
    double corner_min[3]{};
    double corner_max[3]{};
    ...
    void expand_to_include_AABB(const AABB_it& other) {
        for (int i = 0; i < 3; ++i){
...
struct BVH_Node_it {
    ...
    // Unique pointers to prevent memory leaks with raw pointers and new BVH_Node_it use
    std::unique_ptr<BVH_Node_it> left_child;
    std::unique_ptr<BVH_Node_it> right_child;
    //BVH_Node_it* left_child {nullptr}; // Nullptr if leaf.
    //BVH_Node_it* right_child {nullptr};
...
// Structure is the same as BVH_Node_it, so this isn't exactly necessary and would save me having to write the splitting function twice. 
// Here for clarity as I am trying to work out how this works.
struct TLAS_Node_it {
    AABB_it bounding_box {};
    std::unique_ptr<TLAS_Node_it> left_child;
    std::unique_ptr<TLAS_Node_it> right_child;
...
struct TLAS_it{
    std::vector<std::reference_wrapper<BVH_it>> mesh_blas; // Reference wrappers to avoid copying the BLASes themselves
    std::unique_ptr<TLAS_Node_it> root;
...
bool binned_sah_split_t_it(TLAS_Node_it& Node,
 ...
 void build_bvh_it(BVH_Node_it& Node,
    const std::vector<std::array<double,3>>& mesh_triangle_centroids,
    const std::vector<AABB_it>& mesh_triangle_aabbs,
